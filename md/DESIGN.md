...
*   *仕様*: ステージ開始時は「攻撃機能」はロックされており、**ステージ内で最初のパリィを成功させて初めて攻撃が開始される**。
...
*   **攻撃の個性**: 敵個体ごとに固有の射撃スタイルを持つ。
    *   *チャージショット*: 溜め動作の後の高速弾。
    *   *まっすぐ*: 基本的な直進弾幕。
    *   *不規則*: 軌道が変わるトリッキーな弾。
    *   *薙ぎ払いレーザー*: 画面を薙ぎ払うように発射されるレーザー。
*   **武器変異**: どの雑魚敵をどう倒すか、あるいはどの攻撃をパリィするかによって、武器の特性（例: ホーミング機能の付与など）が変化する。各武器は2種類程度の個性を組み合わせて多彩なレパートリーを持たせる。
